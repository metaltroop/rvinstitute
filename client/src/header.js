...
export default function Header() {
  ...
  useEffect(() => {
    fetch("https://frail-housecoat-toad.cyclic.app/profile", {
      credentials: "include",
    }).then((response) => {
  ...
  function logout() {
    fetch("https://frail-housecoat-toad.cyclic.app/logout", {
      credentials: "include",
      method: "POST",
    });
    setUserInfo(null);
    ...
    <div className="m-8 ">
      <header className="flex justify-evenly bg-[#97cadb4c] py-6 px-20 rounded-full backdrop-filter backdrop-blur-lg blurBack font-[Poppins]">
        ...
        >
          ...
          <Link
            to="/blog"
            className="hidden text-xl text-gray-200 hover:text-[#ffffff] transition-all hover:delay-100"
          >
            Blog
          </Link>
          <a
          ...
          {!username && (
            <>
              <Link
                className="hidden bg-[#d7d7d7ec] hover:bg-[#ffffffe9] px-6 py-2 rounded-full transition-all hover:delay-100"
                to="/login"
              >
                Login
              </Link>
              <Link
                className="hidden bg-[#d0d0d0ec] hover:bg-[#ffffffe9] px-6 py-2 rounded-full transition-all hover:delay-100"
                to="/register"
              >
                Register
              </Link>
